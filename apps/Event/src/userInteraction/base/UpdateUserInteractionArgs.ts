/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UserInteractionWhereUniqueInput } from "./UserInteractionWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { UserInteractionUpdateInput } from "./UserInteractionUpdateInput";

@ArgsType()
class UpdateUserInteractionArgs {
  @ApiProperty({
    required: true,
    type: () => UserInteractionWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => UserInteractionWhereUniqueInput)
  @Field(() => UserInteractionWhereUniqueInput, { nullable: false })
  where!: UserInteractionWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => UserInteractionUpdateInput,
  })
  @ValidateNested()
  @Type(() => UserInteractionUpdateInput)
  @Field(() => UserInteractionUpdateInput, { nullable: false })
  data!: UserInteractionUpdateInput;
}

export { UpdateUserInteractionArgs as UpdateUserInteractionArgs };
